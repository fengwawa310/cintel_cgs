(function($){
 ...
 $.fn.changeSingleSelectContent = function(settings){
	...
	if(settings.unitCode==null||settings.unitCode==""){
		var strFunc = $.trim($("#"+$cId).attr("title"));
		settings.unitCode = strFunc;
	}
	...
	$.each(settings.valItem,function(i){
		...
		htmlArray[i]= '<div class="singleSelectCheckDiv'+selectStr+'" onmouseenter="singleSelectItemMouseEnter(\''+$cId+'\',this,event)" onclick="singleSelectClickItem(\''+$cId+'\',this,\''+settings.unitCode+'\')"><span class="singleSelectCheckValue" style="display:none;">'+itemv+'</span><span style="float:left;margin-left:5px;margin-top:9px;" class="singleSelectCheckSpan">'+itemt+'</span></div>';				
	});
  ...
  $.initSingleSelect = function(settings){	
	 ...
	 var strFunc = "";
	 if(settings.unitCode!=null&&settings.unitCode!=""){
		strFunc = settings.unitCode;
	 }
 	 var selHtml = '<div style="display:none;" id="'+settings.id+'" title="'+strFunc+'"></div><div><div class="singleSelect-loader" id="'+settings.id+'_singleSelect_loader">数据加载中...</div><input type="hidden" name="'+settings.unitCode+'" id="'+settings.id+'cgh" value=""/><input type="text" class="singleSelect-input verification"  name="'+settings.unitName+'" style="width:'+settings.width+'" id="'+settings.id+'_singleSelect_input" onkeyup="singleSelectInputKeyUp(\''+settings.id+'\',this,event)" id="'+settings.id+'_singleSelect_img" onclick="singleSelectContentShow(\''+settings.id+'\',event)"/>';
 ...
 function singleSelectItemMouseEnter(id,obj,eve){
  ...
  function singleSelectClickItem(cId,obj,func){
	  var cInput = $("#"+cId+"_singleSelect_input");
	  var cInput_hidden = $("#"+cId+"cgh");
	  //alert(cInput_hidden.val())
	  var cTxt = $.trim($(obj).find(".singleSelectCheckSpan").html());
	  
	  var cTxt_hidden = $.trim($(obj).find(".singleSelectCheckValue").html());
	  
   	  cInput.val(cTxt);
	  cInput_hidden.val(cTxt_hidden);
	  //alert(cInput_hidden.val())
	  $("#"+cId+"_singleSelect_content .singleSelectCheckDiv").removeClass("singleSelectCheckDiv_select");
