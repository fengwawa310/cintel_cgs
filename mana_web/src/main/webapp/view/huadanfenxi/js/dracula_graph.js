...
Graph.Renderer.Raphael.prototype = {
    ...
    draw: function() {
        ...
        for (var i = 0; i < this.graph.edges.length; i++) {
            ...
            var iwo = this.graph.edges[i].source.edges;
            var iwoID = this.graph.edges[i].source.id
            var coloR = "#ffc107";
            var color = "#0a67b1";
            if( iwo.length > 2){
                $("ellipse").each(function () {
                    if( $(this).attr("id") == iwoID ){
                        $(this).attr({fill:coloR, "stroke-width": 2});
                    }
                })
            }
        }
    ...
    drawNode: function(node) {
        ...
        if(!node.render) {
            node.render = function(r, node) {
                ...
                //     var edgNo = node.edges[i].source.edges
                //     for( var y = 0 ; y < edgNo.length;y++ ){
                //         var Noedglength = edgNo[y].source.edges.length
                //         if( Noedglength <= 2 ){
                //         }else{
                ...
                //     }
                // }
                var ellipse = r.ellipse(0, 0, 30, 20).attr({fill: color, stroke: "", "stroke-width": 2});
        ...
        shape.translate(Math.round(point[0]-(box.x+box.width/2)+10),Math.round(point[1]-(box.y+box.height/2)+10))
